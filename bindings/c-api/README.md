...
`include/lorepia.h` is the stable Windows interop contract. ABI version 2
...
JSON event batches contain both `events` and `dropped_events`. When the latter
is non-zero, callers refresh the persisted conversation instead of attempting
to reconstruct missed token deltas. Credentials are accepted only as
per-command input and are never persisted by the Rust core.
